...
-- Define hostname and personal desktop hostname

local helpers = {}

-- Check if the current machine is the personal desktop
helpers.is_personal_desktop = function()
    local handle = io.popen("hostname")
    assert(handle, "Failed to get hostname")
...
helpers.move_mouse_onto_focused_client = function(c)
...
helpers.start_monitor = function(script, callbacks)
    ...
    local kill_monitor_script =
        string.format([[ pkill %s ]], helpers.get_main_process_name(script))
...
helpers.change_volume = function(change_by)
    local percentage = change_by < 0 and string.format("-%s%%", -change_by)
...
helpers.change_brightness = function(change_by)
    local cmd = change_by < 0 and "light -U " .. -change_by
...
helpers.switch_language = function()
    awful.spawn.easy_async_with_shell("ibus engine", function(stdout)
        local curr_index =
            helpers.get_language_map_index("engine", stdout:gsub("%s+", ""))
...
-- Focus or open a client by class
helpers.focus_or_open = function(identifier, spawn_cmd)
    for _, c in ipairs(client.get()) do
        if c.class == identifier or c.name == identifier then
