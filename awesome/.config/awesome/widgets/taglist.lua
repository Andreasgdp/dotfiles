...
local buttons = function(screen)
    return gears.table.join(
        awful.button({}, keys.leftclick, function(t)
            t:view_only()
        end),

        awful.button({ keys.modkey }, keys.leftclick, function(t)
            if client.focus then
                client.focus:move_to_tag(t)
            end
        end),

        awful.button({}, keys.rightclick, function(t)
            if client.focus then
                client.focus:move_to_tag(t)
            end
        end),

        awful.button({ keys.modkey }, keys.rightclick, function(t)
            if client.focus then
                client.focus:move_to_tag(t)
            end
        end),

        awful.button({}, keys.scrolldown, function(t)
            awful.tag.viewprev(t.screen)
        end),

        awful.button({}, keys.scrollup, function(t)
